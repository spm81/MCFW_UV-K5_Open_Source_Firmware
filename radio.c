...
VfoState_t VfoState[4];

#ifdef ENABLE_CW
	const char *modulationTypeOptions[6] = {" FM", " AM", "SSB", "CW", "BYP", "RAW"};
#else
...
const char *bwNames[4] = {"  25k", "12.5k", "8.33k", "6.25k"};
...
if (gNextTimeslice500ms == 1)
			{	// <= 3 seconds TX time left .. start beeping

			if  (gNextTimeslice500ms & 1u)
				{
